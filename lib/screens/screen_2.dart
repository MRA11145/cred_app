...
class _Screen2State extends State<Screen2> with TickerProviderStateMixin {
  ...
  EMIPlanModel selectedEMIPlan = EMIPlanModel(label: 'RECOMMENDED', amount: '₹5,117', duration: '24 months'); // default EMI plan
  Screen2Provider provider = Screen2Provider();

  @override
  void initState() {
    StackPopupModel.incCurrentStackPopupIndex();
    slideController = AnimationController(vsync: this, duration: const Duration(seconds: 1));
    slideAnimation = Tween<double>(begin: 0.0, end: 0.9).animate(slideController);
    super.initState();
  }
      ...
      child: Stack(
        children: [
          AnimatedSwitcher(
            ...
            child: Consumer<Screen2Provider>(builder: (context, provider, child) {
              return provider.getIsEmiClicked() ? _stackPopupView() : _originalView();
            }),
          ),
  ...
  Widget _stackPopupView() {
    return Align(
      ...
      child: Container(
        ...
        key: ValueKey('stackPopupKey' "${StackPopupModel.getCurrentStackPopupIndex()}"),
  ...
  Widget _originalView() {
    return GestureDetector(
      ...
      child: Column(
        key: ValueKey('originalViewKey' "${StackPopupModel.getCurrentStackPopupIndex()}"),
        children: [
          Row(
            children: [
              SizedBox(
    ...
    return Column(
      ...
      children: [
        ...
        Row(
          ...
          children: [
            CommonWidgets.FontWidget(StringConstants.emiPlanHeader, Colors.white.withOpacity(0.8), FontWeight.w500, "Roboto", FontStyle.normal, 80, TextAlign.left),
            SizedBox(
  ...
  Widget _getRateButton() {
    return Container(
      ...
      child: Center(child: CommonWidgets.FontWidget("@1.49% p.m", Colors.green, FontWeight.w700, "Roboto", FontStyle.normal, 50, TextAlign.center)),
    );
  }

  // Widget for the subheader
  Widget _getSubHeader() {
    return CommonWidgets.FontWidget(StringConstants.oneRate, Colors.white.withOpacity(0.6), FontWeight.w400, "Roboto", FontStyle.normal, 60, TextAlign.left);
  }

  // Widget for displaying selected EMI amount
  Widget _selectedAmountWidget() {
    List<Widget> li = [];
    li.add(CommonWidgets.FontWidget(StringConstants.emi, Colors.white.withOpacity(0.5), FontWeight.w400, "Inter", FontStyle.normal, 60, TextAlign.left));
    li.add(CommonWidgets.FontWidget(selectedEMIPlan.amount, Colors.white.withOpacity(0.7), FontWeight.w500, "Inter", FontStyle.normal, 70, TextAlign.left));
  ...
  Widget _durationWidget() {
    List<Widget> li = [];
    li.add(CommonWidgets.FontWidget(StringConstants.duration, Colors.white.withOpacity(0.5), FontWeight.w400, "Inter", FontStyle.normal, 60, TextAlign.left));
    li.add(CommonWidgets.FontWidget(selectedEMIPlan.duration, Colors.white.withOpacity(0.7), FontWeight.w500, "Inter", FontStyle.normal, 70, TextAlign.left));
      ...
      child: SizedBox(
        ...
        child: Padding(
          ...
          child: Row(
            mainAxisAlignment: MainAxisAlignment.center,
            children: [
              CommonWidgets.FontWidget(StringConstants.chooseEMI, Colors.black, FontWeight.w600, "Inter", FontStyle.normal, 70, TextAlign.left)
            ],
          ),
        ),
      ),
